...
const stepColors: Record<number, { bg: string; text: string }> = {
  ...
  3: { bg: "bg-emerald-500/10", text: "text-emerald-400" },
  4: { bg: "bg-violet-500/10", text: "text-violet-400" },
  5: { bg: "bg-rose-500/10", text: "text-rose-400" },
  6: { bg: "bg-amber-500/10", text: "text-amber-400" },
};
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        {userTier === "free" && (
          <Link
            to="/settings"
            className="text-xs text-ocean-400 hover:text-ocean-300 transition-colors font-medium"
          >
            Upgrade to Pro
          </Link>
        )}
      ...
      <div className="grid grid-cols-3 2xl:grid-cols-6 gap-3">
        {skills.map((skill) => {
          const locked = skill.tier === "pro" && userTier === "free";
          const lastRun = skillLastRun?.[skill.id];
          return (
            <SkillCard
              key={skill.id}
              skill={skill}
...
function SkillCard({
  ...
  const colors = stepColors[skill.step] || {
    bg: "bg-ocean-500/10",
    text: "text-ocean-400",
  };
  const isComingSoon =
  ...
  return (
    <div
      className={`relative flex flex-col p-4 rounded-xl border transition-colors h-[230px] ${
        locked
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <div
          className={`w-10 h-10 rounded-lg ${locked ? "bg-dark-700" : colors.bg} flex items-center justify-center ${locked ? "text-ocean-600" : colors.text}`}
        >
          {icon}
        </div>
        <div className="flex items-center gap-1.5 flex-wrap justify-end">
          ...
          {locked && (
            <span className="flex items-center gap-1 text-[10px] uppercase tracking-wider bg-ocean-900/50 text-ocean-400 px-1.5 py-0.5 rounded-full leading-none">
              <LockIcon />
              Pro
            </span>
          )}
      ...
      {/* Skill name */}
      <h3
        className={`text-sm font-medium mb-1 ${locked ? "text-ocean-500" : "text-ocean-100"}`}
      >
        {skill.name}
      </h3>
      ...
      {/* Description */}
      <p
        className={`text-xs leading-relaxed ${locked ? "text-ocean-600" : "text-ocean-400"}`}
      >
        {skill.description}
      </p>

      {/* Trigger prompt as quote */}
      {skill.triggers?.[0] && (
        <p className={`mt-auto text-[11px] italic leading-snug ${locked ? "text-ocean-700" : "text-ocean-400"}`}>
          "{skill.triggers[0]}"
        </p>
      )}

      {/* Footer: run status + step number */}
      <div className="flex items-center justify-between mt-2 pt-2 border-t border-dark-700">
        {lastRun ? (
          ...
          <span className="text-[10px] text-ocean-600">Not run yet</span>
        )}
        <span
          className={`text-[10px] font-mono ${locked ? "text-dark-600" : "text-ocean-600"}`}
        >
